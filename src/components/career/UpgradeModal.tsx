...
interface UpgradeModalProps {
  isOpen: boolean;
  onClose: () => void;
  featureName?: string; // which feature triggered the upgrade prompt
}
...
const PRO_FEATURES = [
  'Everything in Free',
  'Help Me Apply (AI application assistant)',
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="rounded-3xl overflow-hidden" style={{ background: '#FFFFFF', boxShadow: '0 24px 60px rgba(0,0,0,0.2)' }}>
              ...
              <div className="px-6 pt-6 pb-4 flex items-start justify-between">
                ...
                <button onClick={onClose} className="p-1.5 rounded-full" style={{ background: '#F5F5F5', color: '#888' }}>
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="px-6 pb-6 grid grid-cols-2 gap-4">
                ...
                <div className="rounded-2xl p-4" style={{ background: '#F8F8F8', border: '1px solid #E5E5E5' }}>
                  ...
                  <p className="text-2xl font-bold mb-4" style={{ color: '#111' }}>$0</p>
                  <div className="space-y-2">
                    {FREE_FEATURES.map(f => (
                      <div key={f} className="flex items-start gap-2">
                        <Check size={14} className="shrink-0 mt-0.5" style={{ color: '#22C55E' }} />
                ...
                {/* Pro Plan */}
                <div className="rounded-2xl p-4 relative" style={{ background: '#0095FF', border: '2px solid #0095FF' }}>
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-bold" style={{ background: '#111', color: '#FFFFFF' }}>
                    RECOMMENDED
                  </div>
                  <p className="text-xs font-semibold mb-1" style={{ color: 'rgba(255,255,255,0.7)' }}>PRO</p>
                  <div className="flex items-baseline gap-1 mb-4">
                    <p className="text-2xl font-bold" style={{ color: '#FFFFFF' }}>$9</p>
                    <p className="text-xs" style={{ color: 'rgba(255,255,255,0.7)' }}>/month</p>
                  </div>
                  <div className="space-y-2">
                    {PRO_FEATURES.map(f => (
                      <div key={f} className="flex items-start gap-2">
                        <Check size={14} className="shrink-0 mt-0.5" style={{ color: '#FFFFFF' }} />
